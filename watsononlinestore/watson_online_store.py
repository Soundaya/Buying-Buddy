...
class WatsonOnlineStore:
    def __init__(self, bot_id, slack_client,
        ...
        self.discovery_data_source = os.environ.get(
            'DISCOVERY_DATA_SOURCE', DISCOVERY_STORE)
        try:
    ...
    def setup_conversation_workspace(conversation_client, environ):
        ...
        #    LOG.debug("Using WORKSPACE_ID=%s" % env_workspace_id)
            ...
            name = environ.get('WORKSPACE_NAME', 'bot_poc')
            for workspace in workspaces:
    ...
    def handle_discovery_query(self):
        ...
        if (self.context.get('brand_selection') and self.context.get('small_appliances_choice')):
            ...
            if (self.context['brand_selection'] != "" and self.context['small_appliances_choice'] != ""):
                brand = (self.context['brand_selection']).encode('utf-8')
                product = (self.context['small_appliances_choice']).encode('utf-8')
                query_string = """Brand : "{}", Product :"{}" """.format(brand,product).replace("_", " ")
                self.context['brand_selection'] = ''
                self.context['small_appliances_choice'] = ''

        elif(self.context.get('watches_type') and self.context.get('watches_sex')):

            if (self.context['watches_type'] != "" and self.context['watches_sex'] != ""):
                type = (self.context['watches_type']).encode('utf-8')
                gender = (self.context['watches_sex']).encode('utf-8')
                query_string = """ Gender: "{}", Product :"{}" """.format(gender,type).replace("_", " ")
                self.context['watches_type'] = ''
                self.context['watches_sex'] = ''
            print(query_string)
    ...
    def handle_temporary_discovery_query(self):

        items = " 1. Steam Iron : JCPenny : $14 \n 2. Steam Iron : Macys : $17 \n 3. Steam Iron : Best Buy : $22"
        response = { "discovery_response" : items }
        self.context = self.context_merge(self.context, response)
        return True 
    ...
    def format_discovery_response(response, data_source):
        ...
        def get_product_name(entry):
            ...
            if 'Name' in entry:
                product_name = entry['Name'].encode('utf-8')
        ...
        def get_product_url(entry):
            ...
            if 'URL' in entry:
                product_url = entry['URL'].encode('utf-8')
    ...
    #
    #     brand_list = ["Bella", "Black and Decker", "Bissel"]
    #     formatted_out = "\n".join("{}) {}".format(i + 1, item.encode('utf-8'))
    #                               for i, item in enumerate(brand_list))
    #     formatted_list = {'brand_list': formatted_out}
    #     self.context = self.context_merge(self.context, formatted_list)
    #     return True

    # def get_products(self):
    #
    #     product_list = ["Steam Iron", "Vacuum Cleaner", "Panini Grill", "Microwave", "Rice Cooker"]
    #     formatted_out = "\n".join("{}) {}".format(i + 1, item.encode('utf-8'))
    #                               for i, item in enumerate(product_list))
    #     formatted_list = {'product_list': formatted_out}
    #     self.context = self.context_merge(self.context, formatted_list)
    #     return True
    ...
    def list_categories_available(self):
        ...
        formatted_list = {'category_list': formatted_out}
        self.context = self.context_merge(self.context, formatted_list)
        return True
    ...
    #    LOG.debug("watson_response:\n{}\n".format(watson_response))
        ...
        if (self.context.get('debug') == 'yes'):
            print('I am Here. Start Debugging')

        sender.send_message("\n".join(watson_response['output']['text']) +
                            "\n")
        
        list_category = self.context.get('list_category')
        if list_category == "yes":
            output = self.list_categories_available()
            if output == True:
                sender.send_message(self.context.get('category_list'))
                self.context['list_category'] = ''
            else:
                sender.send_message("\n", join("Could not find the suitable Categories. Please Try Later..") + "\n")


        brands = self.context.get('brand')
        if brands == 'yes':
            return self.get_brand_names()

        products = self.context.get('product')
        if products == 'yes':
            return self.get_products()


        if (self.context.get('discovery_string') and self.discovery_client):
            if self.context['discovery_string'] == 'yes' :
                ...
                if output == True :
                    ...
                    sender.send_message("Sorry Discovery service seems to be unavailable now. Try later.")
            #return self.handle_temporary_discovery_query()

        cart_action = self.context.get('shopping_cart')
        if cart_action == 'list':
            output = self.handle_list_shopping_cart()
            if output == True :
                sender.send_message(self.context.get('shopping_cart'))
                sender.send_message("How Do you want to proceed?")
                self.context['shopping_cart'] = ""
            else:
                sender.send_message("Sorry the database service seems to be unavailable now. Try later.")
